...
from vocab import PAD_ID, UNK_ID, _PAD, _UNK
from pretty_print import print_example

from modules import HighWayNetwork, biLSTMEncoder, SimpleSoftmaxLayer, \
                    BiDAFAttn, Modeling_layer_biLSTM_Encoder, \
                    Output_layer_biLSTM_Encoder
...
class QAModel(object):
    ...
    def __init__(self, FLAGS, id2word, word2id, emb_matrix, 
        ...
        with tf.variable_scope("QAModel", initializer=tf.contrib.layers.variance_scaling_initializer(factor=1.0, uniform=True)):
            ...
            self.add_embedding_layer(emb_matrix, emb_alphabet)
            self.build_graph()
            self.add_loss()
    ...
    def add_placeholders(self):
        ...
        if (self.FLAGS.use_char_based_embeddings):
    ...
    def add_embedding_layer(self, emb_matrix, emb_alphabet):
        """
        ...
        with vs.variable_scope("embeddings"):
            ...
            if (self.FLAGS.use_char_based_embeddings):
                ...
                with vs.variable_scope("char_embeddings"):
                    ...
                    # Character embedding matrix (trainable) definition
                    init = tf.constant(np.random.randn(len(emb_alphabet), self.FLAGS.char_emb_size), dtype=tf.float32)
                    char_embedding_matrix = tf.get_variable("char_emb_matrix", initializer=init, dtype=tf.float32)
                        # 1st and 2nd rows are not trainable (constant <unknown> char and <pad> char)
                    unk_char = tf.constant(np.random.randn(1, self.FLAGS.char_emb_size), dtype=tf.float32)
                    pad_char = tf.constant(np.random.randn(1, self.FLAGS.char_emb_size), dtype=tf.float32)

                    char_embedding_matrix = tf.concat([unk_char, pad_char, char_embedding_matrix], axis=0)
                    
                    # Character embeddings are obtained from embedding matrix lookup of character ids placeholders
...
#                    filters = self.FLAGS.char_based_embedding_size # filters = number of convolution filters = dimension of char-based word embeddings
#                    kernel_size = self.FLAGS.kernel_size # size of filter used for convolution
                    
                    # kernel sizes = filter sizes for convolution over char embeddings
                    min_kernel_size = 3
                    max_kernel_size = 5
                    kernel_sizes = range(min_kernel_size, max_kernel_size + 1)
                    
                    # number of filters for ea. type of n-grams
                    filters = self.FLAGS.char_based_embedding_size

                    context_charbased_embs_list = []
                    qn_charbased_embs_list = []
                    for kernel_size in kernel_sizes:
                        with vs.variable_scope("conv_maxpool-%s" % kernel_size):
                            self.context_feat_vectors = tf.layers.conv2d(self.context_char_embs, filters=filters, kernel_size=(1, kernel_size), strides=1)
                            self.qn_feat_vectors = tf.layers.conv2d(self.qn_char_embs, filters=filters, kernel_size=(1, kernel_size), strides=1)
                                                 # shape (batch_size, seq_len, word_len - kernel_size + 1, filters)
                                 # Character-based embeddings for context words
                            self.context_charbased_embs = tf.layers.max_pooling2d(self.context_feat_vectors, pool_size=(1, self.context_feat_vectors.shape[2]), strides=1) # (batch_size, seq_len, 1, filters)
                            self.context_charbased_embs = tf.squeeze(self.context_charbased_embs, axis=2) # (batch_size, seq_len, filters)
                                 # Character-based embeddings for question words
                            self.qn_charbased_embs = tf.layers.max_pooling2d(self.qn_feat_vectors, pool_size=(1, self.qn_feat_vectors.shape[2]), strides=1)
                            self.qn_charbased_embs = tf.squeeze(self.qn_charbased_embs, axis=2)
                             
                            context_charbased_embs_list.append(self.context_charbased_embs)
                            qn_charbased_embs_list.append(self.qn_charbased_embs)
                     
                    qn_charbased_embs_list = tf.concat(qn_charbased_embs_list, axis=2)
                    context_charbased_embs_list = tf.concat(context_charbased_embs_list, axis=2)
                     
                                                 
                     # *** Word and char-based word embeddings concatenation ***
                    self.context_embs = tf.concat([self.context_embs, context_charbased_embs_list], axis=2)
                                 # shape (batch_size, context_len, embedding_size+char_based_embedding_size)
                    self.qn_embs = tf.concat([self.qn_embs, qn_charbased_embs_list], axis=2)
                                 # shape (batch_size, question_len, embedding_size+char_based_embedding_size)
    ...
    def get_chars(self, batch):
        """
        Gets arrays of characters in order to feed self.context_char_ids and self.qn_char_ids placeholders

        Inputs:
          batch: a Batch object

        Returns:
          context_char_ids: numpy array of shape (batch_size, context_len, word_len)
          qn_char_ids: numpy array of shape (batch_size, question_len, word_len)
        """

        # Get word tokens from batch ids
        current_batch_size = len(batch.qn_ids) # last batch may be smaller than batch_size
        qn_words = [[self.id2word.get(idx, UNK_ID) for idx in batch.qn_ids[batch_idx]] for batch_idx in range(current_batch_size)]
        cn_words = [[self.id2word.get(idx, UNK_ID) for idx in batch.context_ids[batch_idx]] for batch_idx in range(current_batch_size)]
        
        # Get characters (char) from word tokens and change directly into char_ids (char2id.get(char, UNK_ID))
        context_char_ids = [[[self.char2id.get(char, UNK_ID) for char in cn_words[batch_idx][word_idx] if \
          cn_words[batch_idx][word_idx] not in [_PAD, _UNK]] for word_idx in range(self.FLAGS.context_len) ] \
            for batch_idx in range(current_batch_size) ]
        
        qn_char_ids = [[[self.char2id.get(char, UNK_ID) for char in qn_words[batch_idx][word_idx] if \
          qn_words[batch_idx][word_idx] not in [_PAD, _UNK]] for word_idx in range(self.FLAGS.question_len) ] \
            for batch_idx in range(current_batch_size) ]

        # Pad lists of char ids with PAD_ID in order to have fixed length word_len
        context_char_ids_padded = [[context_char_ids[batch_idx][word] + [PAD_ID for _ in range(self.FLAGS.word_len - len(context_char_ids[batch_idx][word]))] \
                            for word in range(self.FLAGS.context_len)] for batch_idx in range(current_batch_size)]
        
        qn_char_ids_padded = [[qn_char_ids[batch_idx][word] + [PAD_ID for _ in range(self.FLAGS.word_len - len(qn_char_ids[batch_idx][word]))] \
                            for word in range(self.FLAGS.question_len)] for batch_idx in range(current_batch_size)]

        # Un-nest lists into numpy arrays
        context_char_ids_padded = np.array([[[context_char_ids_padded[batch_idx][word_idx][char_idx] for char_idx in range(self.FLAGS.word_len)] \
                                              for word_idx in range(self.FLAGS.context_len)] for batch_idx in range(current_batch_size)])
        qn_char_ids_padded = np.array([[[qn_char_ids_padded[batch_idx][word_idx][char_idx] for char_idx in range(self.FLAGS.word_len)] \
                                         for word_idx in range(self.FLAGS.question_len)] for batch_idx in range(current_batch_size)])
        
        return context_char_ids_padded, qn_char_ids_padded
    ...
    def run_train_iter(self, session, batch, summary_writer):
        ...
        if (self.FLAGS.use_char_based_embeddings):
            context_char_ids, qn_char_ids = self.get_chars(batch)
            input_feed[self.context_char_ids] = context_char_ids
            input_feed[self.qn_char_ids] = qn_char_ids
    ...
    def get_dev_loss(self, session, dev_context_path, dev_qn_path, dev_ans_path):
        ...
        for batch in get_batch_generator(self.word2id, dev_context_path, dev_qn_path, dev_ans_path, self.FLAGS.batch_size, 
                                         context_len=self.FLAGS.context_len, question_len=self.FLAGS.question_len, discard_long=True):
    ...
    def check_f1_em(self, session, context_path, qn_path, ans_path, dataset, num_samples=100, print_to_screen=False):
        ...
        for batch in get_batch_generator(self.word2id, context_path, qn_path, ans_path, self.FLAGS.batch_size, 
                                         context_len=self.FLAGS.context_len, question_len=self.FLAGS.question_len, discard_long=False):
            ...
            for ex_idx, (pred_ans_start, pred_ans_end, true_ans_tokens) in enumerate(zip(pred_start_pos, pred_end_pos, batch.ans_tokens)):
                ...
                if print_to_screen:
                    print_example(self.word2id, batch.context_tokens[ex_idx], batch.qn_tokens[ex_idx], batch.ans_span[ex_idx, 0],
                                  batch.ans_span[ex_idx, 1], pred_ans_start, pred_ans_end, true_answer, pred_answer, f1, em)
    ...
    def train(self, session, train_context_path, train_qn_path, train_ans_path, dev_qn_path, dev_context_path, dev_ans_path):
        ...
        while self.FLAGS.num_epochs == 0 or epoch < self.FLAGS.num_epochs:
            ...
            # Loop over batches
            for batch in get_batch_generator(self.word2id, train_context_path, train_qn_path, train_ans_path, self.FLAGS.batch_size,
                                             context_len=self.FLAGS.context_len, question_len=self.FLAGS.question_len, discard_long=True):
                ...
                if global_step % self.FLAGS.eval_every == 0:
                    ...
                    train_f1, train_em = self.check_f1_em(session, train_context_path, train_qn_path, train_ans_path, "train", 
                                                          num_samples=1000)
